...
- 关键词 + 向量的混合检索，以及查询改写
- `search_reports`、`search_tables`、`extract_table`、`list_reports` 等工具
- 单 Agent runtime，支持工具调用、引用生成和工具轨迹
- FastAPI 后端，提供文档、会话、普通问答和 SSE 流式问答接口
...
`SESSION_DB_PATH` 可选；不设置时默认使用 `data/sessions.sqlite3`。该文件属于本地运行数据，已加入 `.gitignore`。
...
表格工具默认读取 `data/processed/tables.json`。该文件是表格检索与抽取工具使用的表格索引；如果本地没有准备它，`search_tables` 和 `extract_table` 会返回空结果，但正文检索链路仍然可用。
...
-> ChromaDB
-> HybridRetriever
-> tools(search_reports/search_tables/extract_table)
-> SingleAgentRuntime
...
flowchart TD
    ...
    C2 --> C3["chunks.json"]
    C4["tables.json<br/>optional table index"]
